...
module Errors (Printer : Printer.Printer) = struct
  ...
  let print_spanned_warning (msg : string) ?(span_msg : string option)
      (span : Pos.t) : unit =
    Printer.warning_print "%a" format_structured_error (msg, [ (span_msg, span) ])

  let print_multispanned_warning (msg : string)
      (spans : (string option * Pos.t) list) =
    Printer.warning_print "%a" format_structured_error (msg, spans)
end
